    ...
    with st.chat_message("assistant", avatar="🎓"):
        with st.spinner("Đang tìm kiếm tài liệu và tổng hợp câu trả lời..."):
            try:
                ...
                # Truyền lịch sử để câu hỏi nối tiếp ("còn học bổng thì sao?")
                # được ghép ngữ cảnh TRƯỚC khi retrieve. Không có bước này thì
                # embedding của câu cụt gần như vô nghĩa, retrieval trả về rác.
                # Bỏ lượt cuối vì đó chính là câu đang hỏi.
                ...
                # Cho người dùng thấy hệ thống hiểu câu nối tiếp của họ thành gì.
                rewritten = response.get("search_query", query)
                if rewritten and rewritten.strip() != query.strip():
                    st.caption(f"🔎 Đã hiểu câu hỏi thành: *{rewritten}*")
